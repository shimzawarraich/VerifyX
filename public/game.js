// --- CONFIG ---
// Replace with your actual Gradient AI endpoint and API key (use env vars in production)
const GRADIENT_API_URL = 'https://api.gradient.ai/v1/chat/completions';
const GRADIENT_API_KEY = 'YOUR_GRADIENT_API_KEY'; // ← swap this out
...
// Level-specific prompts: as levels increase, ask for harder-to-detect examples
...
async function startRound() {
  ...
  try {
    ...
    // Graceful fallback: use a static question so the game never breaks
...
async function fetchQuestion() {
  ...
  // Rotate through types so each session has variety
  const type = types[Math.floor(Math.random() * types.length)];
  const difficulty = LEVEL_DIFFICULTY[currentLevel] || LEVEL_DIFFICULTY[1];
...
Return ONLY valid JSON — no markdown, no code fences, nothing else. Format:
{
  ...
  const response = await fetch(GRADIENT_API_URL, {
    ...
    body: JSON.stringify({
      model: 'llama3-8b-instruct', // Update to whichever model your Gradient account exposes
      max_tokens: 300,
      temperature: 0.85,
      messages: [
...
function showComplete() {
  ...
  // If on last level, hide "next level" button
  if (currentLevel >= 5) {
